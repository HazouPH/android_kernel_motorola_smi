...
static void sst_do_recovery(struct intel_sst_drv *sst)
{
	...
	/*
	 * There are some bug reports that board hangs when
	 * sst recovery happens, because LPE is still working.
	 * Here we delay a couple of seconds before returning,
	 * then upper layer would call runtime pm _put to shutdown
	 * LPE.
	 */
	msleep(3000);
}
