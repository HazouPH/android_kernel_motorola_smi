			 ...
			 int type)
{
	...
	if (type & (MMC_BLK_READ | MMC_BLK_WRITE)) {
		host->failures++;
		pr_warn("%s: %d card failures\n", mmc_hostname(host),
				host->failures);
		if (!(host->caps & MMC_CAP_NONREMOVABLE) &&
		    host->failures >= MMC_MAX_FAILURES) {
			pr_err("%s: removing card due to excessive "
				"failures\n", mmc_hostname(host));
			mmc_card_set_removed(host->card);
			mmc_detect_change(host, 0);
			return -ENODEV;
		}
	} else if (md->reset_done & type)
		return -EEXIST;
